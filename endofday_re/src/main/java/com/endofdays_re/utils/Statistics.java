...
public enum Statistics {
    ...
    public static void dump() {
        ModUtils.debug("### Spawner ###");
        for (int i = 0; i < SPAWNER_STATS.size(); i++) {
            SpawnerStat stat = SPAWNER_STATS.get(i);
            ModUtils.debug("Rule " + i + " spawned " + stat.counter + " mobs");
        }
        ModUtils.debug("### Spawn ###");
        for (int i = 0; i < SPAWN_STATS.size(); i++) {
            SpawnStat stat = SPAWN_STATS.get(i);
            ModUtils.debug("Rule " + i + " fired " + stat.counter + " times (" + (stat.deny ? "deny)" : "allow)"));
        }
    }
